...
```
ne semble que fonctionnner avec des fichiers .csv 
créer une liste composée de sous-listes décrivants les métadonnées du fichier : path, profile, encoding, name, format, mediatype et schema. Dans la sous-liste "schema" sont distingués les "fields" du dataset (nom des headers) auxquels sont attribués un "name",un "type" et "format", et les missing values.
